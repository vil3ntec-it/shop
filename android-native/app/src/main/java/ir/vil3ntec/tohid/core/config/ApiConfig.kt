...
object ApiConfig {
  ...
  /**
   *  پیشوندِ بی‌نسخه — همان چیزی که سرور از روزِ اول هم می‌شناسد.
   *
   *  ── چرا اصلاً دوتاست ───────────────────────────────────────────────
   *  سرور هر دو را سوار می‌کند: `/api/…` و `/api/v1/…`. ولی در نسخه‌های
   *  پیش از شهریور، `/api` **اول** سوار می‌شد و درخواستِ `/api/v1/x` را
   *  هم همان می‌قاپید؛ داخلش مسیری به نامِ `/v1/x` نبود، پس به آخرین
   *  لایه (دفترِ داده) می‌رسید که توکن و دکان می‌خواهد — و جوابش ۴۰۴ یا
   *  ۴۰۱ بود. یعنی روی آن سرورها **هیچ‌کدام** از مسیرهای `/api/v1`
   *  کار نمی‌کرد: نه ساختنِ حساب، نه کدِ پیوستن، نه حتی `/me`.
   *
   *  سرور درست شد، ولی سرورِ هر دکان‌دار همان روز به‌روز نمی‌شود. پس
   *  برنامه هم یاد گرفت: اگر نسخه‌دار جواب نداد، همین را امتحان می‌کند و
   *  هر کدام کار کرد، تا پایانِ نشست همان می‌ماند.
   */
  const val API_PREFIX_PLAIN: String = "/api"
   ...
   */
  fun urlOf(base: String, path: String, prefix: String = API_PREFIX): String {
