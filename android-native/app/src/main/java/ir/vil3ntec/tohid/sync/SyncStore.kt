...
class SyncStore(context: Context) {
  ...
  fun signOut() {
    /*
     *  ── چیزی که اینجا بود و برداشته شد ────────────────────────────
     *  تا دیروز پیش از پاک کردن، توکنِ تازه‌سازی کنارِ نامِ حساب در
     *  `SavedLogins` گذاشته می‌شد تا «ورودِ سریع» بدونِ رمز کار کند.
     *  یعنی «خروج از حساب» یک کلیدِ زندهٔ نودروزه — و رمزنشده — روی
     *  گوشی جا می‌گذاشت. شرحِ کامل سرِ `SavedLogins`.
     *
     *  حالا هرچه از حساب مانده می‌رود: توکن از حافظهٔ رمزشده پاک
     *  می‌شود و هر توکنی که نسخه‌های قبلی در فهرستِ حساب‌ها نوشته‌اند
     *  همین‌جا شسته می‌شود.
     *
     *  ⚠️ باطل کردنِ نشست **روی سرور** کارِ این تابع نیست — اینجا
     *  شبکه‌ای در کار نیست. صدازننده باید پیش از این،
     *  `AuthRepository.signOut()` را بزند؛ `Profile` همین را می‌کند.
     *  ترتیبش هم مهم است: اول سرور، بعد اینجا، وگرنه توکنی که باید
     *  باطل شود پاک شده و دیگر چیزی برای فرستادن نیست.
     */
    runCatching { SavedLogins.purgeTokens(app, force = true) }
    tokens.clear()
    prefs.edit()
