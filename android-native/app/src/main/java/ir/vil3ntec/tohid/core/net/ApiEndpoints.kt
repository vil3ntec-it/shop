...
object ApiEndpoints {
   ...
   *
   *  تا دیروز تنها راهِ گرفتنِ اشتراک این بود که طرف واتساپ بزند و
   *  صاحب سامانه هم همان لحظه پشتِ پنل باشد. حالا کد را می‌زند و
   *  اشتراکش همان‌جا فعال می‌شود.
   */
  object Vip {
    const val REDEEM = "/vip/redeem"
  }

  /* ------------------------------ پشتیبانِ ابری ------------------------------ */

  /**
   *  پشتیبانِ همین دکان، روی سرور.
   *
   *  ── چرا لازم شد ──────────────────────────────────────────────────
   *  `AutoBackup` هر شب سه نسخه روی **خودِ گوشی** می‌گذاشت. آن‌ها
   *  پاک شدنِ اشتباهی و خرابیِ دفتر را می‌گیرند — ولی گوشی که گم یا آب
   *  شود، با خودش می‌برَدشان. و پشتیبانِ دستی هم به حافظهٔ آدمی بسته
   *  بود که سرش شلوغ است.
   *
   *  ⚠️ فهرست **مالِ همین دکان** است و شناسه‌اش از توکن می‌آید، نه از
   *  درخواست — پس هیچ دکانی پشتیبانِ دکانِ دیگری را نمی‌بیند.
   */
  object Backups {
    const val ROOT = "/me/backups"
    fun one(id: String) = "/me/backups/$id"
  }
   ...
   *
   *  ⚠️ تا امروز تنها جای ثبتش `/support/push` بود، یعنی برای گرفتنِ
   *  خبرِ «کالا تمام شد» باید از درِ **پشتیبانی** وارد می‌شدی. اسمش
   *  هیچ ربطی به کارش نداشت و هیچ برنامه‌ای پیدایش نکرده بود.
   */
  object Push {
    const val ROOT = "/me/push"
  }
   ...
   *  گفت‌وگو با پشتیبانی.
   *
   *  توکن اجباری نیست: کسی که هنوز حساب نساخته و همان اول کار گیر
   *  کرده، بیشتر از همه به این نیاز دارد. بی‌توکن، گفت‌وگو به شناسهٔ
   *  دستگاه بسته می‌شود و هر وقت حساب ساخت، به حسابش می‌چسبد.
   */
  object Support {
    const val THREAD = "/support/thread"
    const val MESSAGES = "/support/messages"
    const val READ = "/support/read"
    const val PUSH = "/support/push"
  }
   ...
   *
   *  بدونِ این، صاحب سامانه فقط کسانی را می‌دید که ثبت‌نام کرده بودند؛
   *  یعنی دقیقاً کسانی که دیگر لازم نبود دنبالشان برود.
   */
