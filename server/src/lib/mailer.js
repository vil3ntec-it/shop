 ...
 *
 * ── چرا `log` هم «آماده» نیست ─────────────────────────────────────
 * `log` پیش‌فرض است و کد را فقط در لاگِ سرور چاپ می‌کند؛ به هیچ صندوقی
 * نمی‌رسد. اگر اینجا «آماده» شمرده می‌شد، صفحه‌ی خانه‌ی برنامه‌ی مدیریت
 * هیچ هشداری نمی‌داد و صاحب سامانه تا وقتی کاربری شکایت نمی‌کرد
 * نمی‌فهمید هیچ کدِ ثبت‌نامی بیرون نمی‌رود — همان چیزی که تا امروز
 * می‌گذشت.
 */
function readiness(s) {
  const missing = [];
  if (s.provider === 'log') {
    missing.push('راه ارسال (SMTP یا سرویس) — الان کد فقط در لاگ سرور چاپ می‌شود');
