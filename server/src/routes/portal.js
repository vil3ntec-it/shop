...
async function reportErrorHandler(app, req, res, next) {
  try {
    const tenantId = await tenantOf(app, req);
    const id = newId('cer');
    //  ⚠️ همان جدولِ `/api/errors` — یک دفترِ خطا، نه دو تا. `at` ستونِ
    //  مشترکِ زمان است و `created_at` برای سازگاری با همین مسیر می‌ماند.
    const t = now();
    await query(
      `INSERT INTO client_errors (id, app, user_id, tenant_id, version, platform, message, stack, context, at, created_at)
      ...
      [id, app, req.user.id, tenantId,
        ...
        JSON.stringify(v.payload(req.body?.context ?? {}, { max: 8 * 1024 })), t]
    );
