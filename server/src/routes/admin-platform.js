  ...
  try {
    ...
    try {
      const out = await mailer.send({
        ...
        //  همان مدلی که به مشتری می‌رود، تا مدیر همان را ببیند که مشتری می‌بیند
        html: require('../lib/mail-templates').messageHtml({
          app: String(req.body?.app || 'shop'),
          title: 'ایمیل کار می‌کند',
          body: 'این یک پیام آزمایشی از سرور توحید است.\n\nحالا کد ثبت‌نام و کد اشتراک هم به همین شکل برای کاربران می‌رود.',
        }) || mailer.card({
          title: 'ایمیل کار می‌کند',
