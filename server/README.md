...
## بی PostgreSQL و بی داکر — PGlite
...
همان PostgreSQL است، داخلِ خودِ Node (WASM)، با یک پوشه روی دیسک. هیچ SQLی
عوض نمی‌شود — کلِ آزمون‌ها با `npm run test:pglite` روی همین راه‌انداز
می‌دوند و CI هر دو را می‌سنجد. پشتیبان با `dumpDataDir` یک `.pglite.tar.gz`
است (بازگردانی: سرور خاموش، پوشه خالی، `PGlite.create({ loadDataDir })`).
مدیر را هم می‌شود از محیط داد تا کسی پشتِ ترمینال لازم نباشد:
`ADMIN_BOOTSTRAP_USER` / `ADMIN_BOOTSTRAP_PASSWORD`.

راهِ واقعیِ کامپیوترِ خانگی همین است: پنلِ سرورِ خانگی (ریپوی `server`) این
سرور را با همین تنظیم به عنوانِ پروسهٔ فرزند بالا می‌آورد.

## روی سرورِ خانگی، پشتِ پنل (راهِ واقعیِ استقرار)

دامنهٔ `api.vill3n.top` مالِ تونلِ پنلِ خانگی (ریپوی `server`) است و آن پنل
هرچه مالِ این سرور است را از همان دامنه به این‌جا می‌برد. پس Caddy و `DOMAIN`
لازم نیست؛ فقط:

```bash
cp .env.example .env      # رمزها را عوض کنید؛ DOMAIN خالی بماند
#   TRUST_PROXY=true      # ⚠️ لازم — IPِ واقعیِ کاربر از پنل می‌آید
docker compose up -d      # پورت ۳۰۰۰، فقط روی 127.0.0.1
```

سنجه: `https://api.vill3n.top/api/health` باید `"server":"online"` و نسخهٔ همین
`package.json` را بدهد. پنلِ مدیریت از اینترنت: `https://api.vill3n.top/admin/`.
